...
namespace InventorySystem1._0
{
    public partial class frmStockOut : Form
    {
        ...
        {
            TotAmount.Text = "0";
            for (int i = 0; i < dtCus_addedlist.RowCount; i++)
            {
                TotAmount.Text = Convert.ToString(double.Parse(TotAmount.Text) + double.Parse(dtCus_addedlist.Rows[i].Cells[5].Value.ToString()));
            }
            string stockoutID;
            ...
            sql = "INSERT INTO `tblor` ( `TRANSACTIONNUMBER`, `TRANSACTIONDATE`, `SUPLIERCUSTOMERID`, `TOTAL`)" +
                " VALUES ('" + stockoutID + "','" + DateTime.Now.ToString("yyyy-MM-dd") + "','" + txt_cusid.Text +
                "','" + TotAmount.Text + "')";
            config.Execute_Query(sql);
...

        {
...

            {
                ...
                if (dtCus_addedlist.RowCount == 0)
                {
                    ...
                    tot = double.Parse(dtgCus_itemlist.CurrentRow.Cells[3].Value.ToString()) * Convert.ToInt32(qty1.Text);
        ...
        {
            int sum = Convert.ToInt32(dtgCus_itemlist.CurrentRow.Cells[6].Value.ToString());
